...
#function for cleaning unnecessary objects from the scene so they don't interfere with new program run. It also cleans the data for propper input later on
def cleaner():
...
#function for rotating objects so the longest side will be aligned with global X axis. After it's done, function names the objects based on their X  length
def rotate_along_x():      
...
#function spaces sorted objects
...
#function sorts list of objects based on first X and than Y dimmensions from shortest to longest
def sorted_objects():
...
# function creates text object corresponding to block of same elements, places it and fills with data (ammount of same objects, length, width and height). It also calculates location data necessary for placement of frames engulfing blocks of objects. Finally it writes output to tex file
def add_text_obj(obj, prev_l, prev_w, prev_h, counter, tot_len, is_it_last, y_loc):
	...
	tex.location.x = 0.1
        ...
        #function rounds up the values and represents them as interger if the number is whole
	def castrate(n):
		nint = int(n)
		nfloat = round(n, 2)
		if nint/n == 1:
...
#function both draws frames and creates text objects with data according to changes in object sizes
...
#function draws the frames based on input parameters
